...
public class EstudianteRepositoryImplement implements EstudianteRepository {
    ...
    public List<EstudianteDTO> getAllEstudiantesPorCiudad(String ciudad) {
        String jpql = "SELECT new Integrador2.DTO.EstudianteDTO(e.DNI, e.nombre, e.apellido, e.genero, e.edad, e.numLegajo, e.ciudad) FROM Estudiante e WHERE e.ciudad = :ciudad";
    ...
    public EstudianteDTO getEstudiantePorLegajo(Long numLegajo) {
        String jpql = "SELECT new Integrador2.DTO.EstudianteDTO(e.DNI, e.nombre, e.apellido, e.genero, e.edad, e.numLegajo, e.ciudad) FROM Estudiante e WHERE e.numLegajo= : numLegajo";
        Query query = this.em.createQuery(jpql);
    ...
    public List<EstudianteDTO> getAllEstudiantesPorGenero(String genero) {
        String jpql = "SELECT new Integrador2.DTO.EstudianteDTO(e.DNI, e.nombre, e.apellido, e.genero, e.edad, e.numLegajo, e.ciudad) FROM Estudiante e WHERE e.genero = :genero";
        Query query = this.em.createQuery(jpql);
    ...
    public List<EstudianteDTO> getAllEstudiantesOrdenadoPorEdad() {
        TypedQuery<EstudianteDTO> estudiantes = this.em.createQuery("SELECT new Integrador2.DTO.EstudianteDTO(e.DNI, e.nombre, e.apellido,e.genero, e.edad,e.numLegajo,e.ciudad) FROM Estudiante e ORDER BY e.edad DESC", EstudianteDTO.class);
